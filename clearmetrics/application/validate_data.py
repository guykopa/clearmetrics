...
class ValidateDataUseCase:
    """Orchestrates data quality validation after verifying the user has validation rights."""

    def __init__(self, quality_service: QualityService) -> None:
        self._quality_service = quality_service

    def execute(self, records: list[dict[str, Any]], user_role: str) -> list[QualityResult]:
        """Run all quality rules against records for an authorized user."""
        if user_role not in _ALLOWED_ROLES:
            raise PermissionError(f"Role '{user_role}' is not allowed to validate data.")
